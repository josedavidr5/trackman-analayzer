...
# ---------------------------------------------------------
# 1. FILE UPLOADER (SUPPORTS MULTIPLE FILES / FOLDERS)
# ---------------------------------------------------------
st.sidebar.title("1. Upload Data")
st.sidebar.info("You can drag and drop multiple CSVs or an entire folder of files here.")
uploaded_files = st.sidebar.file_uploader("Drop Trackman CSV(s) here", type=['csv'], accept_multiple_files=True)

@st.cache_data
def load_data(files):
    # Merge all uploaded files into one giant DataFrame
    df_list = [pd.read_csv(file, low_memory=False) for file in files]
    df = pd.concat(df_list, ignore_index=True)

    # --- SMART COLUMN MAPPING (Handles Game vs Practice Files) ---
    # If a standard column is missing, check for common Trackman alternatives
    col_mapping = {
        'TaggedPitchType': ['AutoPitchType', 'PitchType'],
        'PitchCall': ['Call', 'PitchResult'],
        'Batter': ['BatterName', 'BatterId'],
        'Pitcher': ['PitcherName', 'PitcherId']
    }
    
    for standard_name, alternatives in col_mapping.items():
    ...
    if 'PitchCall' in df.columns:
        ...
        df = df[df['PitchCall'].astype(str) != 'Undefined']
        df = df[df['PitchCall'].astype(str) != 'Warmup']
        
    if 'Batter' in df.columns:
        df = df[df['Batter'].notna()]
        df = df[df['Batter'].astype(str) != 'Warmup']

    # Ensure Pitch Type is never blank
...
# STOP THE APP IF NO FILES ARE UPLOADED
if not uploaded_files:
    st.title("⚾ Trackman Analysis Dashboard")
    st.info("👈 Please upload your Trackman CSV file(s) in the sidebar menu to begin.")
    st.stop()

# Load and merge all uploaded files
raw_df = load_data(uploaded_files)

# ---------------------------------------------------------
# 2. GLOBAL DATE FILTER (SIDEBAR)
# ---------------------------------------------------------
st.sidebar.title("2. Filter Settings")

# Ensure there are valid dates before creating the calendar
if 'Date' in raw_df.columns and not raw_df['Date'].dropna().empty:
    ...
    # Apply the date filter to the master dataframe
    if len(date_range) == 2:
        start_date, end_date = date_range
        # Convert start/end to datetime for accurate filtering
    ...
    df = raw_df
    st.sidebar.warning("No valid dates found in uploaded files to filter.")

# ---------------------------------------------------------
# 3. APP NAVIGATION
# ---------------------------------------------------------
st.sidebar.title("3. Navigation")
dashboard_type = st.sidebar.radio("Select Dashboard:", ["Pitching", "Hitting"])

# ---------------------------------------------------------
# 4. PITCHING DASHBOARD
# ---------------------------------------------------------
if dashboard_type == "Pitching":
    st.title("⚾ Pitching Analysis Dashboard")
    
    if 'Pitcher' not in df.columns or df.empty:
        st.warning("No pitching data found in the selected date range.")
    else:
        all_pitchers = sorted(df['Pitcher'].dropna().unique())
        selected_pitcher = st.selectbox("Select a Pitcher:", all_pitchers)
        
        p_df = df[df['Pitcher'] == selected_pitcher].copy()
        
        if p_df.empty:
            st.warning("No live data found for this pitcher in the selected date range.")
        else:
            st.subheader(f"Data Summary: {selected_pitcher} (Live Pitches: {len(p_df)})")
            ...
            try:
                ...
                st.error("Some metrics (like SpinRate or Breaks) are missing from these game files.")
            ...
            with col1:
                ...
                if 'PlateLocSide' in p_df.columns and 'PlateLocHeight' in p_df.columns:
            ...
            with col2:
                ...
                if 'PlateLocSide' in p_df.columns and 'PlateLocHeight' in p_df.columns and len(p_df.dropna(subset=['PlateLocSide', 'PlateLocHeight'])) > 1:
            ...
            with col_btn2:
                ...
                with PdfPages(pdf_buffer) as pdf:
                    if 'summary' in locals():
                        ...
                        ax_tbl.axis('tight')
                        ax_tbl.axis('off')
                        ax_tbl.set_title(f"Pitching Summary: {selected_pitcher}", fontweight="bold", fontsize=16)
...
# ---------------------------------------------------------
    ...
    st.title("🏏 Hitting Analysis Dashboard")
    
    if 'Batter' not in df.columns or df.empty:
        st.warning("No hitting data found in the selected date range.")
    else:
        all_batters = sorted(df['Batter'].dropna().unique())
        selected_batter = st.selectbox("Select a Batter:", all_batters)
        
        b_df = df[df['Batter'] == selected_batter].copy()
        
        if b_df.empty:
            st.warning("No live data found for this batter in the selected date range.")
        else:
            ...
            with col1:
                ...
                if 'Bearing' in b_df.columns and 'Distance' in b_df.columns:
            ...
            with col2:
                ...
                if 'PlateLocSide' in b_df.columns and 'PlateLocHeight' in b_df.columns:
                    ...
                    ax5.text(0.5, 0.5, 'Missing Plate Location Data', ha='center', va='center')
            ...
            with col_btn1:
                ...
                st.download_button(
                    ...
                    data=csv_hit_data,
                    file_name=f"{selected_batter}_Hitting_Summary.csv",
                    mime='text/csv',
                )
            ...
            with col_btn2:
                ...
                with PdfPages(pdf_buffer) as pdf:
                    ...
                    ax_tbl_hit.axis('off')
                    ax_tbl_hit.set_title(f"Hitting Progression: {selected_batter}", fontweight="bold", fontsize=16)
